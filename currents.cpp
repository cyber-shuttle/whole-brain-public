...
//  double sigma_n = sigma_e * (I-ud*pd*(x+1.2)+0.15*H1(x)-pl*(x+(1-sigma)));
  ...
  double delta=0.24;
  yBeta = y+beta_n;
  //double k = (0.25-(0.25-0.0025));
  if (yBeta>-2.819){
	yBeta = -2.819+(yBeta+2.819)*k;
  }
  
  if(x <= -0.5) {
	x = alpha / (1.0 - x) + yBeta;  // change y + beta_n to ypBeta
  }else{
	if(x < +1.0) {
	  x = +1.;             // change y + beta_n to ypBeta
	  ud = ud+delta;
	  k=0.0025;
	}else {
	  x = -1;
...
double AMPA::calc(double x, double y_pre, int y_post) {
 ...
 I = fac_AMPA_TC* strength * R * (y_pre - E_AMPA);
 return I;
}
...
double AMPA_D1::calc(double x, double y_pre, int y_post) {
 ...
 if(q > Deadtime){
  ...
  }  //the flag that spike was but wasn't utilized
...
double AMPA_D2::calc(double x, double y_pre, int y_post){
 ...
 if(q > Deadtime){ //if it has been long enough seince last spike or mini
  ...
  }else if( ((x - lastrelease1) > 70.0) && ((x - lastrelease) > newrelease) ) {//if it has been long enough since last spike and mini
 ...
 I = g1 * R * (y_pre - E_AMPA); //E_ampa = 0, R = , g1 = g_ampa*E 
...
double AMPAmapD1::calc(double x, double y_pre, int y_post){
 ...
 else if( ((x - lastrelease1) > 100.0) && ((x - lastrelease) > newrelease) ) {   //70.0 is replaced by 70000000.0
...
void TC::calc(double x, double I, double *y, double *f){
 ...
 ICa::calc(y[1], f[1], iT, x);
 f[0] = -G_l * (y[0] - E_l) - iT - ih - iNa - iK - iA - fac_gkl_TC * G_kl * (y[0] - INaK::E_K) + I +DC;
 v_DEND = v_SOMA = y[0];
}
...
void CX_SOMA::calc(double x, double *y, double *f){
 ...
 g2_SOMA = g_Na * INa_CX::E_Na +fac_gkv_cx* g_Kv * IKv_CX::E_Kv + g_Nap * INa_CX::E_Na + 6.74172 + I_Stim2; 
 iSOMA =  - iNa - iKv - iNap;     
}
