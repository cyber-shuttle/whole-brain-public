...
enum Syn_Type{
  ...
  E_GiantAMPA=34,
  E_GiantAMPAMap=35,
  ENUM_Syn_Types=36 // This must be the last entry - to indicate number of synapse types
};
...
class RS:public OtherBase{
  double xp, xpp, mu1, mu2, sigma_n, beta_n;
  double sigma_dc, beta_dc, Tcr;
 public:
  double x, y, alpha, sigma, sigma_e, beta_e, Idc, S_CX_DEND,v_DEND,v_SOMA;
  double bl, sl, ml, pl, pd, ud, k;
  double gamma_ut1, gamma_ut2, d_dep, d_rec, gamma_r, pr, yBeta;
  double gamma1;
  int spike;
...
double H1(double x)
 ...
 void init(){ 
  bl=-0.3;
	...
	pl=0.5;
	pd=5.5;
//const double alpha = 3.65,sigma = 0.02;
	alpha = 3.65;
	sigma = -0.1;
	gamma_ut1=0.995;
	gamma_ut2 = 0.9;
	...
	pd= 1.0; // 5.5; //5.5	
	pl = 1.0; // 0.15; // parameter 4 -- FOR Mara -- change by 10 or 20%
	sigma = -0.4; //  -0.4*pl;
	beta_e=0.133;
	beta_dc=0.133;
	sigma_e=1.0;
...
public: 
 ...
 void init(){  
   ...
   sigma_e=0.5; //1.0; 
   sigma_dc=1.0; 
   beta_e=0.05; //0.1; // Giri: can reduce this value
   beta_dc=0.1;
   Idc = -0.; 
   S_CX_DEND = 165.0e-6; 
   xpp=-1+sigma; 
...
public:
  ...
  CX_DEND(double V0, double Cai0) :IHVA_CX(V0), IKCa_CX(Cai0), IKm_CX(V0),
    ...
    G_l = 1.0e3/30000; //0.06; //1.0e3/30000;  //  mS/cm^2 //SWS-maxim
    
    IKm_CX::G_Km = 0.01; //0.02; //0.01;  //SWS-maxim
